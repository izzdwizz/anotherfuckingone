...
function Trades() {
	...
	//const {text1} = useStateContext(); const {setText1} = useStateContext();
	const [text1, setText1] = useState('');
	const { getTrade } = useStateContext();
	...
	const tradeInfo = amount + '3xLev' + '80%' + '30D' + username;
	// button
	...
	const getKey = async (e) => {
		...
		axios
			...
			.then((response) => {
				setText1(response.data.tradeKey);
			})
			.catch((error) => {
				console.error({ error });
			});
